/* ai-draw :: runtime-ppt.js (forked from html-ppt)
...
(function () {
  ...
  ready(function () {
    ...
    const CHANNEL_NAME = 'ai-draw-ppt-presenter-' + location.pathname;
    let bc;
    ...
    function openPresenterWindow() {
      ...
      presenterWin = window.open('', 'ai-draw-ppt-presenter', 'width=1280,height=820,menubar=no,toolbar=no');
      if (!presenterWin) {
    ...
    function buildPresenterHTML(deckUrl, slideMeta, total, startIdx, channelName, currentTheme) {
      ...
      const storageKey = 'ai-draw-ppt-presenter:' + location.pathname;
  ...
  timerCount.textContent = (idx + 1) + ' / ' + total;
    ...
    if (!themeBase) {
      ...
      if (existingLink) {
        ...
        themeBase = lastSlash >= 0 ? rawHref.substring(0, lastSlash + 1) : 'assets/themes-ppt/';
      } else {
        themeBase = 'assets/themes-ppt/';
      }
    }
